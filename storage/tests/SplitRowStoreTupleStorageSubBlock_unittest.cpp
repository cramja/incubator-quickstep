...
using namespace quickstep::splitrow_internal;
...
TEST_P(SplitRowStoreTupleStorageSubBlockTest, GetCopyGroupsForAttributeMapTest) {
  ...
  std::vector<splitrow_internal::ContiguousAttrs>& contiguous_attrs = copy_groups.contiguous_attrs_;
  std::vector<splitrow_internal::VarLenAttr>& varlen_attrs = copy_groups.varlen_attrs_;

  const std::size_t size_of_string = dst_store->getRelation().getAttributeById(3)->getType().maximumByteLength();

  // Fixed length attributes.
  EXPECT_EQ(0, contiguous_attrs[0].source_attr_id_);
  EXPECT_EQ(4, contiguous_attrs[0].bytes_to_advance_);
  EXPECT_EQ(4, contiguous_attrs[0].bytes_to_copy_);

  EXPECT_EQ(1, contiguous_attrs[1].source_attr_id_);
  ...
  if (testVariableLength()) {
    ...
    EXPECT_EQ(2, contiguous_attrs[2].source_attr_id_);
