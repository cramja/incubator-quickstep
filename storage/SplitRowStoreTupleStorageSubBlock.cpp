...
struct BasicInsertInfo {
  ...
  BitVector<true> is_nullable_;

  std::vector<std::size_t> fixed_len_offsets_;
  std::vector<std::size_t> fixed_len_sizes_;
...
std::size_t SplitRowStoreTupleStorageSubBlock::EstimateBytesPerTuple(
  ...
  // TODO(marC): this could be cached in the header, if the value is
...
tuple_id SplitRowStoreTupleStorageSubBlock::bulkInsertTuples(ValueAccessor *accessor) {
  ...
  for (attribute_id attr_id = 0; attr_id < relation_.size(); ++attr_id) {
