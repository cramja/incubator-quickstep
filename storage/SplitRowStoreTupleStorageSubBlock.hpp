...
//
struct CopyGroup {
  attribute_id source_attr_id_;  // The attr_id of starting input attribute for run.

  CopyGroup(const attribute_id source_attr_id)
...
struct CopyGroupList {
  ...
  void merge_contiguous() {
    ...
    for (int idx = 1; idx < contiguous_attrs_.size(); ++idx) {
      ...
      // The merge step:
      if (previous_attr->source_attr_id_ + 1 == current_attr->source_attr_id_ &&
